...
em [`CHALLENGE.md`](./CHALLENGE.md). Para decisões técnicas, trade-offs e limitações, ver
...
Todos os endpoints HTTP (exceto `/health/*`) exigem um Bearer token JWT válido emitido pelo Keycloak
(realm `jungle-gaming`, importado automaticamente do `docker-compose.yml`). Para obter
...
Detalhes da decisão de auth em `ARCHITECTURE.md`, seção 8.

## Documentação da API (Swagger/OpenAPI)

Com a API no ar, a doc interativa (`@nestjs/swagger`) está em `http://localhost:3000/docs` — gerada a
partir dos DTOs e decorators dos controllers, sempre em sincronia com o código. Endpoints, request/response
schemas, status codes (200/201/202/400/409) e o requisito do header `Idempotency-Key` estão documentados
ali. Use o botão "Authorize" (canto superior direito) com um token do Keycloak (ver seção Autenticação
acima) para testar direto pela UI. O JSON puro (OpenAPI 3) está em `/docs-json`.

## Testando a API

Uma collection pronta está em [`docs/api-collection.json`](./docs/api-collection.json) — formato
Postman Collection v2.1, importável direto no **Postman** (`Import → File`) e também no **Insomnia**
(`Import Data → From File`, que lê Postman v2.1 nativamente). Cobre todos os endpoints abaixo,
incluindo casos de replay idempotente, conflito de idempotência, `PENDING_REFERENCE`, saldo
...
primeiro e cole o `access_token` da resposta na variável `token` (variável de coleção no Postman,
de ambiente no Insomnia); as pastas **Wallets** e **Wagering Transactions** já usam
`Bearer {{token}}` automaticamente em todas as requests.
